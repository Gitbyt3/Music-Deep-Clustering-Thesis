...
const Output1 = () => {
  ...
  const trackData = surveyData['Results1']
  ...
  useEffect(() => {
    if (surveyData.Output1) {
      setFormData(surveyData.Output1)}
  ...
  return (
    <div>
      <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:'40px'}}>
        ...
        <div>
          <h2>Arousal-Valence Output #1</h2>
          <h2 style={{fontWeight:'normal'}}>(Arousal, Valence) → <strong>({surveyData['Input1']['arousal']}, {surveyData['Input1']['valence']})</strong></h2>
        </div>
      </div>
      ...
      <PageNavigation onNavigate={() => updateSurveyData('Output1', formData)} nextDisabled={!isFormComplete()}/>
    </div>
  )
}
